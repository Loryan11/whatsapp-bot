...
module.exports = HandleMsg = async (aruga, message) => {
    try {
        ...
        const isBlacklist = blackList.includes(pengirim)
        const isStaffBot = staffList.includes(pengirim)
	...
	// Filter Banned People
        ...
        case 'speed':
        case 'ping':
            if (isBlacklist) return aruga.reply(from, '♪ Sorry, lu masuk blacklist jadi gabisa ngapa-ngapain!', id)
            await aruga.sendText(from, `♪ Kecepatan Goyangan Bot\nSpeed: ${processTime(t, moment())} _Second_`)
            break
        case 'tnc':
            if (isBlacklist) return aruga.reply(from, '♪ Sorry, lu masuk blacklist jadi gabisa ngapa-ngapain!', id)
        ...
        case 'credit':
            if (isBlacklist) return aruga.reply(from, '♪ Sorry, lu masuk blacklist jadi gabisa ngapa-ngapain!', id)
            await aruga.sendText(from, menuId.textZeonBOT())
            break
        case 'donate':
        case 'donasi':
            if (isBlacklist) return aruga.reply(from, '♪ Sorry, lu masuk blacklist jadi gabisa ngapa-ngapain!', id)
        ...
        case 'join':
            if (isBlacklist) return aruga.reply(from, '♪ Sorry, lu masuk blacklist jadi gabisa ngapa-ngapain!', id)
            if (args.length == 0) return aruga.reply(from, `♪ Jika kalian ingin mengundang bot ke grup silahkan invite atau \nKetik ${prefix}join [link group]`, id)
            let linkgrup = body.slice(6)
            let islink = linkgrup.match(/(https:\/\/chat.whatsapp.com)/gi)
            let chekgrup = await aruga.inviteInfo(linkgrup)
            if (!islink) return aruga.reply(from, '♪ Invalid Group Link!\nPeriksa link grup lagi ;)', id)
            if (isOwnerBot) {
                await aruga.joinGroupViaLink(linkgrup)
                      .then(async () => {
                          await aruga.sendText(from, '♪ Berhasil Join Grup Via Link^_^')
                          await aruga.sendText(chekgrup.id, `♪ Alo semuanya, kenalin gua Zeon-BOT.\nSkuy, cari tau apa saja fitur bot ini dengan ngetik ${prefix}help.\nJangan nelfon bot ini kalo gamau di block ma Zeon-BOT ini^_^`)
                      })
            } else {
                let cgrup = await aruga.getAllGroups()
                if (cgrup.length > groupLimit) return aruga.reply(from, `♪ Maaf, kapasitas grup bot udah penuh :(\nKapasitas grup bot cuma ${groupLimit} aja^_^`, id)
                if (cgrup.size < memberLimit) return aruga.reply(from, `♪ Maaf, Bot ga bisa join karena member kurang dari ${memberLimit}`, id)
                await aruga.joinGroupViaLink(linkgrup)
                      .then(async () =>{
                          await aruga.reply(from, '♪ Berhasil Join Grup Via Link^_^', id)
                      })
                      .catch(() => {
        ...
        case 'botstat': {
            ...
            aruga.sendText(from, `♪ Bot Status :\n- *${loadedMsg}* Loaded Messages\n- *${groups.length}* Group Chats\n- *${chatIds.length - groups.length}* Personal Chats\n- *${chatIds.length}* Total Chats`)
            break
        }
        ...
        case '#rate':
            ...
            if (!rating) tobz.reply(from, '♪ Format salah!\nKetik *!menu* untuk melihat caranya.')
        ...
        case '#apakah':
            ...
            if (!nanya) tobz.reply(from, '♪ Format salah! Ketik *!menu* untuk melihat caranya.')
         ...
         case '#bisakah':
            ...
            if (!bsk) tobz.reply(from, '♪ Format salah! Ketik *!help* untuk melihat caranya.')
        ...
        case 'stiker':
            ...
            if ((isMedia || isQuotedImage) && args.length === 0) {
                ...
                .then(() => {
                    aruga.reply(from, '♪ Nohh stikernya...')
                ...
                if (!isUrl(url)) { await aruga.reply(from, '♪ Maaf, link yang kamu kirim invalid.', id) }
                aruga.sendStickerfromUrl(from, url).then((r) => (!r && r !== undefined)
                    ? aruga.sendText(from, '♪ Maaf, link yang kamu kirim tidak memuat gambar.')
                    : aruga.reply(from, '♪ Noh stikernya...')).then(() => console.log(`Sticker Processed for ${processTime(t, moment())} Second`))
            } else {
                await aruga.reply(from, `♪ Gambar tidak ada! Untuk menggunakan ${prefix}sticker\n\n\nKirim gambar dengan caption\n${prefix}sticker <biasa>\n${prefix}sticker nobg <tanpa background>\n\natau Kirim pesan dengan\n${prefix}sticker <link_gambar>`, id)
            }
            break
        case 'stickergif':
        case 'stikergif':
            ...
            if (isMedia || isQuotedVideo) {
                if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10) {
                    var mediaData = await decryptMedia(message, uaOverride)
                    aruga.reply(from, '♪ Sedang di proses⏳ silahkan tunggu ± 1 min!', id)
                    ...
                    await exec(`gify ${filename} ./media/stickergf.gif --fps=30 --scale=240:240`, async function (error, stdout, stderr) {
                        ...
                        .catch(() => {
                            aruga.reply(from, '♪ Filenya kegedean!', id)
                        })
                    })
                  } else {
        ...
        case 'meme':
            ...
            if ((isMedia || isQuotedImage) && args.length >= 2) {
                ...
                aruga.sendFile(from, ImageBase64, 'image.png', '', null, true)
                    .then(() => {
                        aruga.reply(from, '♪ Nohh, jan lupa react!',id)
                    })
                    .catch(() => {
                        aruga.reply(from, '♪ Ada yang error!')
                    })
            } else {
                await aruga.reply(from, `♪ Gambar tidak ada! Silahkan kirim gambar dengan caption ${prefix}meme <teks_atas> | <teks_bawah>\ncontoh: ${prefix}meme teks atas | teks bawah`, id)
            }
            break
        ...
        case 'jsolat':
            if (isBlacklist) return aruga.reply(from, '♪ Sorry, lu masuk blacklist jadi gabisa ngapa-ngapain!', id)
            if (args.length == 0) return aruga.reply(from, `♪ Untuk melihat jadwal sholat dari setiap daerah yang ada\nketik: ${prefix}jsolat [daerah]\n\nuntuk list daerah yang ada\nketik: ${prefix}daerah`, id)
            const solatx = body.slice(8)
            const solatj = await rugaapi.jadwaldaerah(solatx)
            await aruga.reply(from, solatj, id)
            .catch(() => {
                aruga.reply(from, '♪ Dah input daerah yang ada di list?', id)
            })
            break
        case 'daerah':
            if (isBlacklist) return aruga.reply(from, '♪ Sorry, lu masuk blacklist jadi gabisa ngapa-ngapain!', id)
		...
		case 'facebook':
			...
			.then(async (res) => {
				...
				if (res.status == 'error') return aruga.sendFileFromUrl(from, link, '', 'Sorry, URL-nya ga ada!', id)
				await aruga.sendFileFromUrl(from, linkhd, '', '♪ Nih boss videonya', id)
				.catch(async () => {
					await aruga.sendFileFromUrl(from, linksd, '', '♪ Nih boss videonya', id)
					.catch(() => {
						aruga.reply(from, '♪ Sorry, URL-nya ga ada!', id)
					})
				})
			})
			break
        ...
        case '!infogay':
            if (isBlacklist) return aruga.reply(from, '♪ Sorry, lu masuk blacklist jadi gabisa ngapa-ngapain!', id)
            const quotex = await rugaapi.infogay()
            const mentions = mentionedJidList
            .then(async(res) => {
				await aruga.sendTextWithMentions(from, `${mentions} terdeteksi ${res.persen}% Gay`, '\nNgaku korban lu udah berapa?', '', id)
			})
            .catch(() => {
                aruga.reply(from, '♪ Ada yang Error!', id)
            })
	...
	case 'lirik':
        ...
        case 'xtube':
            if (isBlacklist) return aruga.reply(from, '♪ Sorry, lu masuk blacklist jadi gabisa ngapa-ngapain!', id)
			if (args.length == 0) return aruga.reply(from, `♪ Ingat dosa paok. Bokep aja terus isi otakmu`, id)
			rugaapi.xxxvideo((body.slice(7)))
			.then(async (res) => {
				if (res.status == 'error') return aruga.reply(from, res.hasil, id)
				await aruga.sendFileFromUrl(from, res.link, res.hasil, id)
			})
			break
        case 'whatanime':
            if (isBlacklist) return aruga.reply(from, '♪ Sorry, lu masuk blacklist jadi gabisa ngapa-ngapain!', id)
        ...
        case 'infogempa':
			...
			.then(async (res) => {
                await aruga.reply(from, `${res}`, id)
                await aruga.sendFileFromUrl(from, res.link, 'gempa.jpg', res.hasil, id)
			})
			break
        case 'ceklokasi':
            if (isBlacklist) return aruga.reply(from, '♪ Sorry, lu masuk blacklist jadi gabisa ngapa-ngapain!', id)
	    ...
	    case 'add':
	        ...
	        if (args.length !== 1) return aruga.reply(from, `♪ Untuk menggunakan ${prefix}add\nPenggunaan: ${prefix}add <nomor>\ncontoh: ${prefix}add 628xxx`, id)
                try {
                    ...
                    aruga.reply(from, '♪ Tak Bisa Menambahkan Target', id)
                }
            break
        case 'kick':
            if (isBlacklist) return aruga.reply(from, '♪ Sorry, lu masuk blacklist jadi gabisa ngapa-ngapain!', id)
            ...
            await aruga.sendTextWithMentions(from, `♪ Perintah dijalankan, mengeluarkan:\n${mentionedJidList.map(x => `@${x.replace('@c.us', '')}`).join('\n')}`)
        ...
        case 'promote':
            ...
            await aruga.sendTextWithMentions(from, `♪ Perintah dijalankan, menambahkan @${mentionedJidList[0].replace('@c.us', '')} sebagai admin.`)
            break
        case 'demote':
            if (isBlacklist) return aruga.reply(from, '♪ Sorry, lu masuk blacklist jadi gabisa ngapa-ngapain!', id)
            if (!isGroupMsg) return aruga.reply(from, '♪ Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
        ...
        case 'admin':
            ...
            if (!isGroupMsg) return aruga.reply(from, '♪ Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
            if (!isBotGroupAdmins) return aruga.reply(from, '♪ Gagal, silahkan tambahkan bot sebagai admin grup!', id)
            if (!DevBot) return aruga.reply(from, '♪ Sorry, lu gapunya akses untuk fitur ini')
            if (mentionedJidList.length !== 1) return aruga.reply(from, '♪ Maaf, hanya bisa mempromote 1 user', id)
            if (groupAdmins.includes(mentionedJidList[0])) return await aruga.reply(from, '♪ Maaf, user tersebut sudah menjadi admin.', id)
            if (mentionedJidList[0] === botNumber) return await aruga.reply(from, '♪ Maaf, format pesan salah.\nTidak dapat mempromote akun bot sendiri', id)
            await aruga.promoteParticipant(groupId, mentionedJidList[0])
        ...
        case 'bye':
            if (isBlacklist) return aruga.reply(from, '♪ Sorry, lu masuk blacklist jadi gabisa ngapa-ngapain!', id) 
            if (!isGroupMsg) return aruga.reply(from, '♪ Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
            if (!isOwnerBot) return aruga.reply(from, '♪ Gagal, perintah ini hanya dapat digunakan oleh Owner Bot!', id)
        ...
        case 'everyone':
            if (isBlacklist) return aruga.reply(from, '♪ Sorry, lu masuk blacklist jadi gabisa ngapa-ngapain!', id)
            if (!isGroupMsg) return aruga.reply(from, '♪ Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
            if (!isGroupAdmins) return aruga.reply(from, '♪ Sorry, lu bukan admin grup!')
            const groupMems = await aruga.getGroupMembers(groupId)
            const nom = 0+1
            let hehex = '╔══✪〘 Mention All 〙✪══\n'
            for (let i = 0; i < groupMems.length; i++) {
                hehex += '╠⌬ '
                hehex += ` @${groupMems[i].id.replace(/@c.us/g, '')}\n`
            }
            hehex += '╚═〘 _*☭ Zeon-BOT ☭*_ 〙'
        ...
        case 'blocklist':
            if (isBlacklist) return aruga.reply(from, '♪ Sorry, lu masuk blacklist jadi gabisa ngapa-ngapain!', id)
            if (!isOwnerBot) return aruga.reply(from, '♪ _Sorry, anda bukan owner bot!_')
            let hih = `*Zeon-BOT Blocked Number*\nTotal : ${blockNumber.length}\n`
        ...
        case 'listban':
            if (isBlacklist) return aruga.reply(from, '♪ Sorry, lu masuk blacklist jadi gabisa ngapa-ngapain!', id)
            if (!isOwnerBot) return aruga.reply(from, '♪ _Sorry, anda bukan owner bot!_')
            let hehel = '╔══✪〘 *Banned List* 〙✪══\n'
            for (let i = 0; i < blackList.length; i++) {
                hehel += `╠⌬ `
                hehel += `${i+1}. @${blackList[i]} \n`
            }
            hehel += `╚═〘 _*☭ Zeon-BOT ☭*_ 〙\n\nTotal : ${blackList.length} Members`
            await aruga.sendTextWithMentions(from, hehel)
            break
        case 'grouplist':
            tobz.getAllGroups().then((res) => {
            let gc = '▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁\n▏        Zeon-BOT Group List       ▕\n▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔'
            for (let i = 0; i < res.length; i++) {
                gc += `\n*${i+1}. ${res[i].name}\n ┣ ⌬ Unread Chats : _${res[i].unreadCount}_ Chats\n ┗ ⌬ Not Spam : _${res[i].notSpam}_\n`
            }
		...
		case 'mutegrup':
            ...
            if (args[0] == 'on') {
				aruga.setGroupToAdminsOnly(groupId, true).then(() => aruga.sendText(from, 'Hanya admin yang bisa mengirimkan pesan!'))
			} else if (args[0] == 'off') {
				aruga.setGroupToAdminsOnly(groupId, false).then(() => aruga.sendText(from, 'Silahkan nimbrung kembali!'))
			} else {
        ...
        if (isBlacklist) return aruga.reply(from, '♪ Sorry, lu masuk blacklist jadi gabisa ngapa-ngapain!', id)
        if (!isGroupMsg) return aruga.reply(from, '♪ Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
        if (!isDevbot) return aruga.reply(from, '♪ Siap Laksanakan!')
        ...
        if (!isBotGroupAdmins) return aruga.reply(from, '♪ Gagal, silahkan tambahkan bot sebagai admin grup!', id)
            ...
            aruga.reply(from, '♪ Pembersihan selesai^_^', id)
        break
        
        case 'banall': //mengeluarkan semua member
        if (isBlacklist) return aruga.reply(from, '♪ Sorry, lu masuk blacklist jadi gabisa ngapa-ngapain!', id)
        if (!isGroupMsg) return aruga.reply(from, '♪ Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
        if (isDevbot) return aruga.reply(from, '♪ Sorry, anda tidak memiliki hak akses ini!')
        let isOwner = chat.groupMetadata.owner == pengirim
            const allMem = await aruga.getGroupMembers(groupId)
            for (let i = 0; i < allMem.length; i++) {
                if (groupAdmins.includes(allMem[i].id)) {
                    ...
                    blackList.push(args[1]+'@c.us'+'\n')
                    fs.writeFileSync('./settings/banned.json', JSON.stringify(blackList))
                }
            }
            aruga.reply(from, '♪ Member grup ini seluruhnya terbanned!', id)
        break

        //Owner Bot
        case 'ban':
            if (isBlacklist) return aruga.reply(from, '♪ Sorry, lu masuk blacklist jadi gabisa ngapa-ngapain!', id)
            if (!isOwnerBot) return aruga.reply(from, '♪ Perintah ini hanya untuk Owner bot dan Staff Bot!', id)
            if (!isStaffBot) return aruga.reply(from, '♪ Perintah ini hanya untuk Owner bot dan Staff Bot!', id)
            ...
            if (args[0] == 'add') {
                if (args == DevBot) aruga.reply(from, 'Hayoloh, ngapain mau ban abang ganteng itu?')
                blackList.push(args[1]+'@c.us'+'\n')
                fs.writeFileSync('./settings/banned.json', JSON.stringify(blackList))
                aruga.reply(from, 'Target ter-banned!')
            } else
            if (args[0] == 'del') {
                let xnxx = banned.indexOf(args[1]+'@c.us')
                blackList.splice(xnxx,1)
                fs.writeFileSync('./settings/banned.json', JSON.stringify(blackList))
                aruga.reply(from, 'Target ter-unbanned!')
            } else {
             for (let i = 0; i < mentionedJidList.length; i++) {
                if (mentionedJidList.DevBot) aruga.reply(from, '♪ Hayoloh, ngapain mau ban abang ganteng itu?')
                blackList.push(mentionedJidList[i])
                fs.writeFileSync('./settings/banned.json', JSON.stringify(blackList))
                aruga.reply(from, 'Target ter-banned!', id)
                }
            }
            break
        case 'staff':
            if (isBlacklist) return aruga.reply(from, '♪ Sorry, lu masuk blacklist jadi gabisa ngapa-ngapain!', id)
            if (!isOwnerBot) return aruga.reply(from, '♪ Command ini khusus owner ya dek ;)', id)
            if (args.length == 0) return aruga.reply(from, `Untuk banned seseorang agar tidak bisa menggunakan commands\n\nCaranya ketik: \n${prefix}ban add 628xx --untuk mengaktifkan\n${prefix}ban del 628xx --untuk nonaktifkan\n\ncara cepat ban banyak digrup ketik:\n${prefix}ban @tag @tag @tag`, id)
            if (args[0] == 'add') {
                staffList.push(args[1]+'@c.us'+'\n')
                fs.writeFileSync('./settings/banned.json', JSON.stringify(staffList))
                aruga.reply(from, '♪ Berhasil ditambahkan ke Staff List!')
            } else
            if (args[0] == 'del') {
                let xnxx = banned.indexOf(args[1]+'@c.us')
                staffList.splice(xnxx,1)
                fs.writeFileSync('./settings/banned.json', JSON.stringify(staffList))
                aruga.reply(from, '♪ Dihapus dari Staff List!')
            } else {
             for (let i = 0; i < mentionedJidList.length; i++) {
                staffList.push(mentionedJidList[i])
                fs.writeFileSync('./settings/banned.json', JSON.stringify(staffList))
                aruga.reply(from, '♪ Berhasil ditambahkan ke Staff List!', id)
                }
            }
            break
        case 'bc': //untuk broadcast atau promosi
            if (isBlacklist) return aruga.reply(from, '♪ Sorry, lu masuk blacklist jadi gabisa ngapa-ngapain!', id)
            if (!isOwnerBot) return aruga.reply(from, '♪ Command ini khusus owner ya dek ;)', id)
            if (args.length == 0) return aruga.reply(from, `♪ Untuk broadcast, ketik:\n${prefix}bc [Teks]`)
            let msg = body.slice(4)
            const chatz = await aruga.getAllChatIds()
            const groupz = await aruga.getAllGroups()
            for (let idk of chatz) {
                var cvk = await aruga.getChatById(idk)
                if (!cvk.isReadOnly) aruga.reply(idk, `〘_*♪ Zeon-BOT Broadcast ♪*_〙\n${msg}`)
                if (cvk.isReadOnly) aruga.reply(idk, `〘_*♪ Zeon-BOT Broadcast ♪*_〙\n${msg}`)
            }
            aruga.reply(from, `Broadcast Sukses Terkirim ke:\n ⌬ ${chatz.length} Personal Chat\n ⌬ ${groupz.length} Groups`, id)
            break
        case 'leaveall': //mengeluarkan bot dari semua group serta menghapus chatnya
            ...
            for (let gclist of allGroupz) {
                await aruga.sendText(gclist.contact.id, `♪ Sorry bot lagi bersih-bersih\n Total chat aktif : ${allChatz.length}`)
        ...
        case 'clearall': //menghapus seluruh pesan diakun bot
            if (isBlacklist) return aruga.reply(from, '♪ Sorry, lu masuk blacklist jadi gabisa ngapa-ngapain!', id)
            if (!isOwnerBot) return aruga.reply(from, '♪ Command ini khusus Owner bot ya dek ;)', id)
            ...
            if(find && find.id === groupId){
                ...
                if(cekuser && isIn !== false){
                    if(isKasar){
                        ...
                        if(denda){
                            await aruga.reply(from, "Jangan maki-maki paok\nDenda +5.000\nTotal : Rp"+formatin(denda.denda), id)
                        }
                    }
                } else {
